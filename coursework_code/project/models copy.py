...
#     dashboard_stats = db.relationship('DashboardStats', back_populates='family', lazy=True)
...
#     role = db.Column(db.String(20), nullable=False, default='member')  # admin, owner, member
...
class User(UserMixin, db.Model):
    ...
    transactions = db.relationship('Transaction', back_populates='user', lazy=True, order_by="Transaction.date.desc()")
    user_budgets = db.relationship('UserBudget', back_populates='user', lazy=True)
    dashboard_stats = db.relationship('DashboardStats', back_populates='user', lazy=True, uselist=False)  # Один к одному    
    ...
    #     return Transaction.query.filter_by(user_id=self.id).filter(Transaction.date >= start_date).all()
    ...
    def get_category_breakdown(self, period='all'):
        ...
        for t in transactions:
            if t.amount < 0:  # только расходы
                breakdown[t.category.name] += abs(t.amount)
...
class Category(db.Model):
    ...
    description = db.Column(db.Text, nullable=True)  # Добавьте это поле
...
class CategoryLimit(db.Model):
    ...
    period = db.Column(db.String(10), nullable=False)  # month, year
...
#     daily_balance = db.Column(JSON, default=dict)  # Храним баланс на конец каждого дня
#     last_calculated = db.Column(db.DateTime, default=datetime.utcnow)
    
#     # Отношения
#     family = db.relationship('Family', back_populates='dashboard_stats')
    
#     def __repr__(self):
#         return f'<DashboardStats for family {self.family_id} - {self.period}>'
